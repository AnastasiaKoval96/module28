...
def testing():
    ...
    pytest.driver.get('https://b2c.passport.rt.ru/account_b2c/page?state=22422c7b-4cf8-4a6a-9997-6105df87742c&client_id=account_b2c#/')
...
def test_invalid_phone():
    WebDriverWait(pytest.driver, 10).until(EC.presence_of_element_located((By.ID, 'username')))
    pytest.driver.find_element(By.ID, 'username').send_keys('79125557800')
...
def test_invalid_password():
    WebDriverWait(pytest.driver, 10).until(EC.presence_of_element_located((By.ID, 'username')))
    pytest.driver.find_element(By.ID, 'username').send_keys(valid_email)
    pytest.driver.find_element(By.ID, 'password').send_keys('Koval1996')
    pytest.driver.find_element(By.ID, 'kc-login').click()

# №4 Невалидный логин
def test_invalid_login():
    WebDriverWait(pytest.driver, 5).until(EC.presence_of_element_located((By.ID, 't-btn-tab-login')))
    pytest.driver.find_element(By.ID, 't-btn-tab-login').send_keys('rtkid_1690037928668')
    pytest.driver.find_element(By.ID, 'password').send_keys(valid_password)
    pytest.driver.find_element(By.ID, 'kc-login').click()
...
def test_login_long_line():
    WebDriverWait(pytest.driver, 10).until(EC.presence_of_element_located((By.ID, 't-btn-tab-login')))
    pytest.driver.find_element(By.ID, 't-btn-tab-login').click()
    pytest.driver.find_element(By.ID, 'username').send_keys('jghhhhhhfffffffoooooooooorrrrrrrrrrrr')
    pytest.driver.find_element(By.ID, 'password').send_keys(valid_password)
    pytest.driver.find_element(By.ID, 'kc-login').click()
...
def test_registration_btn():
    ...
    pytest.driver.find_element(By.NAME, 'firstName').send_keys('Анастасия')
    pytest.driver.find_element(By.NAME, 'lastName').send_keys('Пинаева')
    pytest.driver.find_element(By.XPATH, '//*[@id="page-right"]/div/div/div/form/div[2]/div/div/input').send_keys('Санкт-Петербург')
